...
\newpage
```{r echo=FALSE, warning=FALSE, fig.height=3, fig.cap='2018 Monthly Ozone Levels'}
epa_data %>%
  filter(Year == 2018) %>%
  ggplot() +
  ...
  labs(
    x = "Month Number",
    y = expression(paste("Ozone Concentration", sep = " ", "(",mu,g,"/",m^3,")")),
...
```{r echo=FALSE, warning=FALSE, fig.height=3, fig.cap='2019 Monthly Ozone Levels'}
epa_data %>%
  filter(Year == 2019) %>%
  ggplot() +
  aes(x = as.factor(Month),
...
https://github.com/dashiellrs/EDE_Fall2025\
\
\
I promise that I have not used AI to complete any of my work and will not do so in the future. I want to learn this!\
\
