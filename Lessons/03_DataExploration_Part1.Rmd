...
Summarizing the above, your scripts should typically begin with the following
...
str(USGS.flow.data) #structure of df - use to QC data types etc.
dim(USGS.flow.data) #dimensions of df
length(USGS.flow.data)

head(USGS.flow.data) #first 5 records
head(USGS.flow.data, 10) #first 10 records
tail(USGS.flow.data, 5) #last 5 records
USGS.flow.data[30000:30005, c(3, 8, 14)] #specific rows & columns
...
mean(USGS.flow.data$discharge.mean) # if data is missing, mean won't calculate
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") #set column type to date, telling R how to interpret format
```
