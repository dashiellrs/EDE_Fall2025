...
```{r, message = FALSE}
getwd()
library(tidyverse); library(htmltools)
...
Note: assumptions of normality often violated by small sample sizes; run ANOVA anyway.
...
# i.e. only these 2 sites have normally distributed data
# but method is robust from small deviations from normal distribution

# Next test for normality: Q-Q
...
bartlett.test(Litter.Totals$dryMass ~ Litter.Totals$plotID) #continuous var ~ categ. var (levels)

# results: reject null i.e. variances are not equal

# method 1: Format ANOVA as aov
...
# but doesn't tell us which means are different

# method 2: Format ANOVA as lm
...
# Post-hoc test 
# to figure out WHY we rejected the null (i.e. what are the differences among means)
# step 1: TukeyHSD() computes Tukey Honest Significant Differences
# will show you which pairs are significantly different
TukeyHSD(Litter.Totals.anova)

# step 2: Extract groupings for pairwise relationships
# i.e. groups with similar means
# results: treatments w/ same letter are not significantly different
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE)
Litter.Totals.groups #"group" groups levels with the same mean
...
summary(Litter.anova.2way3) #instead of adding, you multiply
...
# Finding treatment groups using with()
# new object with number of levels determined by number of interactions b/w func. group & nlcd class
Litter.interaction <- with(Litter, interaction(functionalGroup, nlcdClass))

#Litter.test <- cbind(Litter,Litter.interaction)

#compare means for each litter interaction levels
