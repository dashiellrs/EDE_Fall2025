...
#Explore as a spatial dataframe...
#View using sf plot
plot(GasPlants['status'],pch=16) #specify column, "pch" sets symbol

#View using ggplot
ggplot(data=GasPlants) + geom_sf(aes(color=status))

#View using mapview
mapview(GasPlants, zcol='status') #"zcol" sets color
...
#Convert to a spatial dataframe by combining mean lat and long
...
mapview(EPAair.sf, cex='mean_PM25') #"cex" sets size
```
...
the_buffer <- sf_GraingerHall %>% st_buffer(1) #1 unit = 1 degree in CRS

#View the result
mapview(the_buffer)

#combined view of buffer and Grainger 
