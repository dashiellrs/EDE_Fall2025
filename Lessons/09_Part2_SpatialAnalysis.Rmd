...
st_crs(epa_pm25_sites_sf) #can add "$epsg" to return only the EPSG
...
ggplot() +
  ...
  geom_sf(data=upper_neuse, fill = "yellow", color = "blue")

mapview(huc8_sf, col.regions = "gray") + 
  mapview(upper_neuse, col.regions = "yellow")
```
...
mapview(counties_sf_join, 
        zcol = 'E_POV150', 
        col.regions = brewer.pal(2, 'RdBu')) +  #specify palette
...
wind_energy <- 
  ...
  mutate(HUC_8 = as.factor(substr(HUC_12,1,8))) %>%
  group_by(HUC_8) %>%
  summarise(Total_Wind_Energy = sum(AvgWindEnergy))
  
#Join to HUC_8 features
HUC8_windenergy_attributes <- merge(
  y = wind_energy,
  x = huc8_sf,
  by.y = "HUC_8",
  by.x = "HUC_8"
...
#View the outputs
ggplot() +
  geom_sf(data=HUC8_windenergy_attributes, aes(fill = Total_Wind_Energy)) +
  labs(fill = "MW/year")
...
state_sf <- counties_sf %>% 
  group_by('STATEFP') %>% 
  summarize(ALAND = sum(ALAND)) #collapsing all counties into single observation of land area

#View the data
mapview(state_sf) #dissolves boundaries
```
...
huc2_sf <- huc8_sf %>%
  group_by(DWQ_Basin) %>%
  summarise(Total_Acres = sum(ACRES), Total_SqMiles = sum(SQ_MILES))

#Map the data
mapview(huc2_sf, zcol = "Total_Acres")
...
#Select intersecting counties using the `st_filter()` command
#st_filter([other sf data frame], .predicate = [predicate command])
#there are lots of predicate options: https://r-spatial.github.io/sf/reference/geos_binary_pred.html
...
TriCo_HUC8_intersect <- huc8_sf_utm %>%
  st_filter(triCo, .predicate = st_intersects)

mapview(TriCo_HUC8_intersect, col.regions = "orange") +
  mapview(triCo, col.regions = "blue")

#Intersect the HUC_8s
TriCo_HUC8_clip <- huc8_sf_utm %>%
  st_intersection(triCo)

mapview(TriCo_HUC8_clip)
```
...
triCo_centroids_chull <- triCo_centroids %>% 
  st_union() %>% #combining points (but not dissolving) for next operation
  st_convex_hull()
...
#Create a boolean mask: to find which county intersects our point
...
#Using matrix notation to find the one feature with "TRUE"
...
theSite_30km <- st_buffer(theSite_sfc_transformed,30000)

theSite_buffer_mask <- st_intersects(counties_sf_utm,
                                     theSite_30km,
                                     sparse = FALSE)

theSite_buffer_counties <- counties_sf_utm[theSite_buffer_mask,]

mapview(theSite_buffer_counties)
