...
library(ggridges) # package for ridge plots
...
O3plot2 <- ggplot(EPAair) +
 # geom_point(aes(x = Date, y = Ozone, color = "blue"))
  geom_point(aes(x = Date, y = Ozone), color = "blue") # move color out of aesthetics
print(O3plot2)
...
PMplot.faceted <-
  ...
  geom_point() + # instead of color, use facet_wrap
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
                  Site.Name == "Bryson City"), # specify a subset
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) # -> column w/ year & rows w/ sites
...
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) + # from 0 to 250
  ylim(0, 20) # from 0 to 20
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() # because we are plotting depth, which goes down
print(tempvsdepth)

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm)  + # add a line to represent relationship b/w nutrients
  geom_abline(aes(slope = 16, intercept = 0)) # define a specific line
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? Fill changes color of inside of box; color changes the outline
print(Nutrientplot3)
...
# "y" tells binaxis which axis
# "center" tells which way to stack the points
# *** "dodge" tells coinciding dots to spread instead of overlay
print(Nutrientplot4)
...
print(Nutrientplot6) # problems: no legend, only one variable on x-axis...

# Using a gathered dataset
Nutrientplot7 <-   
  ggplot(PeterPaul.chem.nutrients.gathered) +
  geom_freqpoly(aes(x = concentration, color = nutrient)) # color inside aesthetics -> adds legend automatically
print(Nutrientplot7)
