...
```{r, fig.height = 3, fig.width = 4}
# distribution of discharge.mean values (histogram)
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))
#SAME AS
ggplot(USGS.flow.data,aes(x = discharge.mean)) + geom_histogram()

# specify bin width
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 5)

# specify number of bins
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# new command: define extent of x axis
...
```{r, fig.height = 3, fig.width = 4}
#frequency poly = histogram but as a curve
#lty for line type
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean), bins = 50) +
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "turquoise") +
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 2, color = "salmon") +
  scale_x_continuous(limits = c(0, 10))

#color is inside aes -> means colors determined by
#the data itself (not just the design of the plot)
ggplot(USGS.flow.data) +
