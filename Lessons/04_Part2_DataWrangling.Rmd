...
# Gather nutrient data into one column using pivot_longer (turn columns into rows)
# need 2 columns: nutrient name, value
...
# subset() is "similar to filter" | is.na() checks if value is NA
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
count(NTL.nutrient.data.PeterPaul.gathered, nutrient) # count() returns number of rows, by column
...
# Spread nutrient data into separate columns (turn rows into columns)
#NTL.nutrient.data.PeterPaul.spread <- spread(NTL.nutrient.data.PeterPaul.gathered, nutrient, concentration)

# Spread nutrient data into separate columns using pivot_wider (undo the gather)
...
LitterTrap <- LitterTrap %>% # pipe to cull unnecessary columns
