...
irradiance.regression <- 
  lm(PeterPaul.chem.nutrients$irradianceWater ~ #lm = linear model (y~x)
...
- Look for symmetry w/ respect to red line
...
- Dashed line = Q-Q for normal distribution
- Circles = Q-Q for data's residuals
- Residuals may naturally deviate a bit at lower and higher ends due to outliers
...
- Values outside dashed lines = outliers
- Drastic departure from red line = poor model fit
- Leverage = impact of individual point on model

```{r, fig.height = 3, fig.width = 4}
par(mfrow = c(2,2), #divide plot screen into 2x2 grid
    mar=c(4,4,4,4)) #define plot sizes
#run this ^^^ then run plot (v) to display all 4 together
plot(irradiance.regression)
par(mfrow = c(1,1)) #return plot screen settings to normal
```
...
Given the distribution of irradiance values, we don't have a linear relationship between x and y in this case. Let's try log-transforming the irradiance values. Note we also remove the observations that seems to be an outlier.

- Remember: to take log, must eliminate values of 0!

```{r, fig.height = 3, fig.width = 4}
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000)
irradiance.regression2 <- lm(
  data = PeterPaul.chem.nutrients, 
  log(irradianceWater) ~ depth) #log-transforming irradiance (y)
...
irradiancebydepth2 <- 
  ...
  scale_y_log10() + #log-transforming irradiance in scatterplot
  geom_point() 
...
irradiancebydepth2 <- 
    ...
    geom_smooth(
      method = 'lm', 
      se = FALSE, #remove confidence interval visual
      color = "black")
print(irradiancebydepth2)
...
Workflow:
1. Generate linear model ('lm')
2. Apply step() function to resulting model
  - Note initial AIC
  - Note change in AIC with removal/addition of single terms
  - If AIC decreases with removal, remove term(s) and re-run 'lm'
  - Repeat: step() will suggest final linear regression model
3. Run suggested model & report finding

``` {r, fig.height = 3, fig.width = 4}
TPregression <- lm(
  data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), #taking just Paul Lake
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit()
PeterPaulCorr <- cor(PeterPaulnutrients) #make correlation matrix
corrplot(PeterPaulCorr, method = "ellipse") #visual version
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #mix numeric & visual
```
...
#consider all nutrients as explanatory variables
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4) 

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC) #does the steps and gives you the coefficients of the remaining explanatory variables 
