...
other_URL <- "https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb"

Eno_gage_data <- read.table(
  other_URL,
  skip = 25,
  header = TRUE,
  sep = '\t',
...
colnames(Eno_gage_data) <- c("agency_cd",	"site_no",	"datetime",	"tz_cd",	"discharge_cfs",	"89062_00060_cd",	"89063_00065",	"89063_00065_cd")
...
US_energy_data <- data.frame(
  "State" = as.factor(the_states),
  "Avg_retail_price_cKwh" = as.numeric(the_price),
  "net_summer_capacity_MW" = as.numeric(gsub(",","",the_capacity)),
  "net_generation_MWh" = as.numeric(gsub(",","",net_generation)),
