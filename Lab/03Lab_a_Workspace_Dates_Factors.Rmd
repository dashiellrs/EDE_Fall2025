...
NTL_LTER_chemphys_data <- read.csv(
...
EPA_files <- dir(raw_data_folder, pattern = "EPAair_O3*.*", full.names = TRUE)
EPA_data <- EPA_files %>%
  map(read.csv) %>%
  reduce(rbind)
...
```{r centuryfix}
#Fix for century issue - specific to lubridate
str_past <- "55-feb-3"
...
## What class is the dow column? numeric ... not right
class(sites$dow)

#List the unique values in the column
unique(sites$dow)

#Summary - is it meaningful? no b/c numeric
summary(sites$dow)
