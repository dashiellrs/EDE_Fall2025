...
# Check for missing observations (missing months)
# Create a data frame with sequence of all months from start point to end point
Months <- as.data.frame(seq.Date(from = as.Date("1901/01/01"),
                                 to = as.Date("2016/12/01"),
                                 by = "month"))
# Rename column in prep for joining with data
colnames(Months) <- "Date"

# left_join() preserves all values in data x --> will make sure all months are represented
BR_complete <- left_join(Months,BR_complete)

# check for NAs now that we are sure we have all months
summary(BR_complete)
...
f_month <- month(min(BR_complete$Date))
f_year <- year(min(BR_complete$Date))
...
Rainfall_SMK1 <- SeasonalMannKendall(Rainfall_ts)
Rainfall_SMK2 <- smk.test(Rainfall_ts)
summary(Rainfall_SMK1)
summary(Rainfall_SMK2)

Temp_SMK1 <- SeasonalMannKendall(Temp_ts)
Temp_SMK2 <- smk.test(Temp_ts)
summary(Temp_SMK1)
summary(Temp_SMK2)
...
Rainfall_LM <- lm(
  data = Rainfall_components,
  trend ~ Date)
summary(Rainfall_LM)

Temp_LM <- lm(
  data = Temp_components,
  trend ~ Date)
summary(Temp_LM)
