...
my_plot <- nutrient_data %>% 
  ...
  ggplot(aes(
    x = sampledate,
    y = !!sym(the_parameter), #tell ggplot to use "the_parameter" as a reference to the actual value
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) +
  scale_x_date(
...
plot_it <- function(the_parameter, start_year, end_year){ #define variables
#Construct the plot
nutrient_data %>% 
  filter(year4 > start_year & year4 < end_year) %>% 
  ggplot(aes(
...
sliderInput(
  ...
  value = c(min(nutrient_data$year4), max(nutrient_data$year4)), #set default
  step = 1,
  round = T,
  sep = "" #remove comma separator bc we're using years
)
